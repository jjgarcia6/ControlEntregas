...
const COLUMNAS: Record<TipoReporte, string[]> = {
  xmls: ["numero_factura", "fecha_emision", "razon_social_emisor", "total_sin_impuestos", "importe_total"],
  kardex: ["fecha_movimiento", "tipo", "origen", "cantidad", "costo_unitario", "costo_total", "saldo_cantidad", "saldo_valor"],
  entregas: ["numero", "fecha_creacion", "snap_nombre", "snap_identificacion", "total_entrega", "saldo_pendiente", "estado"],
  pagos: ["numero_comprobante", "fecha_pago", "banco_nombre", "tipo_cuenta", "nombre_titular", "valor_total"],
};

function buildFilas(tipo: TipoReporte, data: unknown): Record<string, unknown>[] {
  if (!data) return [];
  if (tipo === "xmls") {
  ...
  if (tipo === "kardex") {
    return (data as ReporteKardexResponseType).movimientos.map((m) => ({
      ...
      tipo: m.tipo,
      origen: m.origen,
      cantidad: Number(m.cantidad).toFixed(4),
      costo_unitario: Number(m.costo_unitario).toFixed(4),
      costo_total: Number(m.costo_total).toFixed(2),
      saldo_cantidad: Number(m.saldo_cantidad).toFixed(4),
  ...
  if (tipo === "entregas") {
    return (data as ReporteEntregasResponseType).filas.map((f) => ({
      ...
      snap_nombre: f.snap_nombre,
      snap_identificacion: f.snap_identificacion,
...
export function ReportesContainer() {
  ...
  const { data, isLoading } = useFetchReporte(tipoActivo, filtrosActivos, enabled);
  const { mutate: downloadPdf, isPending: isPdfPending } = useDownloadReportePdf(tipoActivo, filtrosActivos);
  const { mutate: downloadXlsx, isPending: isXlsxPending } = useDownloadReporteXlsx(tipoActivo, filtrosActivos);
  ...
  return (
    <div className="space-y-6">
      ...
      <ReporteFilters key={tipoActivo} tipoActivo={tipoActivo} onSubmit={handleFiltersSubmit} />
