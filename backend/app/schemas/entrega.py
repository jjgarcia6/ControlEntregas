...
class EntregaRequest(BaseModel):
    destinatario_id: uuid.UUID = Field(
        ..., description="ID del destinatario en el catálogo"
    )
...
class EntregaItemResponse(BaseModel):
    ...
    xml_item_id: uuid.UUID = Field(
        ..., description="Ítem de XML de origen del lote consumido"
    )
...
class EntregaResponse(BaseModel):
    ...
    snap_identificacion: str = Field(
        ..., description="Cédula/RUC del destinatario al momento de crear"
    )
    snap_nombre: str = Field(
        ..., description="Nombre del destinatario al momento de crear"
    )
    snap_direccion: str = Field(
        ..., description="Dirección del destinatario al momento de crear"
    )
    snap_telefono: str = Field(
        ..., description="Teléfono del destinatario al momento de crear"
    )
    comentarios: Optional[str] = Field(None, description="Observaciones de la entrega")
    total_entrega: Decimal = Field(
        ..., description="Suma del costo_total de todos los ítems"
    )
    saldo_pendiente: Decimal = Field(
        ..., description="Monto aún no pagado (Fase 5 lo reduce)"
    )
    estado: str = Field(..., description="Estado de la entrega: activa | eliminada")
    items: list[EntregaItemResponse] = Field(
        default_factory=list, description="Ítems de la entrega"
    )
...
class EntregaListItemResponse(BaseModel):
    ...
    snap_identificacion: str = Field(
        ..., description="Identificación del destinatario (snapshot)"
    )
