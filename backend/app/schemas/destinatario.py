...
class DestinatarioCreate(BaseModel):
    ...
    identificacion: str = Field(
        ..., description="Cédula (10 dígitos) o RUC (13 dígitos) válido módulo 11"
    )
    nombre: str = Field(
        ..., min_length=1, max_length=255, description="Nombre completo o razón social"
    )
    direccion: str = Field(..., min_length=1, description="Dirección del destinatario")
    telefono: str = Field(
        ..., min_length=1, max_length=20, description="Teléfono de contacto"
    )
...
class DestinatarioResponse(BaseModel):
    ...
    tipo_identificacion: Literal["cedula", "ruc"] = Field(
        ..., description="Tipo detectado automáticamente: cedula | ruc"
    )
    identificacion: str = Field(
