...
class FiltrosXmls(BaseModel):
    ...
    fecha_desde: date | None = Field(None, description="Inicio del rango de fecha_emision.")
    fecha_hasta: date | None = Field(None, description="Fin del rango de fecha_emision.")
    codigo_principal: str | None = Field(None, description="Filtra ítems por código de producto.")


class FiltrosKardex(BaseModel):
    model_config = ConfigDict(strict=True)
    producto_id: uuid.UUID = Field(..., description="Producto cuyo historial se consulta. Requerido.")
    fecha_desde: date | None = Field(None, description="Inicio del rango de fecha_movimiento.")
    fecha_hasta: date | None = Field(None, description="Fin del rango de fecha_movimiento.")


class FiltrosEntregas(BaseModel):
    model_config = ConfigDict(strict=True)
    fecha_desde: date | None = Field(None, description="Inicio del rango de created_at de entrega.")
    fecha_hasta: date | None = Field(None, description="Fin del rango de created_at de entrega.")
    destinatario_id: uuid.UUID | None = Field(None, description="Filtra por destinatario.")
    estado: Literal["activa", "eliminada"] | None = Field(None, description="Estado de la entrega.")


class FiltrosPagos(BaseModel):
    model_config = ConfigDict(strict=True)
    fecha_desde: date | None = Field(None, description="Inicio del rango de fecha_pago.")
    ...
    entrega_id: uuid.UUID | None = Field(None, description="Filtra pagos que incluyen esta entrega.")
...
class ReporteXmlItemRow(BaseModel):
    ...
    cantidad_documento: Decimal = Field(..., description="Cantidad en la factura original.")
    cantidad_ingresada: Decimal = Field(..., description="Cantidad ingresada al Kardex.")
    cantidad_pendiente: Decimal = Field(..., description="Cantidad aún no ingresada al Kardex.")
...
class ReporteXmlRow(BaseModel):
    ...
    numero_factura: str = Field(..., description="Número de factura (estab-ptoEmi-sec).")
    ...
    total_sin_impuestos: Decimal = Field(..., description="Total sin impuestos del XML.")
...
class ReporteXmlsResponse(BaseModel):
    ...
    total_valor: Decimal = Field(..., description="Suma de importe_total de todos los XMLs.")
...
class ReporteKardexMovimientoRow(BaseModel):
    ...
    saldo_cantidad: Decimal = Field(..., description="Saldo de cantidad acumulado al momento.")
    saldo_valor: Decimal = Field(..., description="Saldo de valor acumulado al momento.")
...
class ReporteEntregaItemRow(BaseModel):
    ...
    costo_unitario: Decimal = Field(..., description="Costo unitario FIFO al momento de entrega.")
...
class ReporteEntregaRow(BaseModel):
    ...
    fecha_creacion: datetime = Field(..., description="Fecha de creación de la entrega.")
...
class ReporteEntregasResponse(BaseModel):
    ...
    total_valor: Decimal = Field(..., description="Suma de total_entrega de todas las filas.")
    total_cobrado: Decimal = Field(..., description="Suma de montos ya cobrados (total_entrega - saldo_pendiente).")
    total_pendiente: Decimal = Field(..., description="Suma de saldo_pendiente de todas las filas.")
    filas: list[ReporteEntregaRow] = Field(..., description="Filas del reporte. Máx 1000.")


# --- Reporte Pagos ---
class ReportePagoDistribucionRow(BaseModel):
    entrega_numero: int = Field(..., description="Número de la entrega receptora.")
    snap_nombre: str = Field(..., description="Nombre del destinatario (snapshot de la entrega).")
...
class ReportePagoRow(BaseModel):
    ...
    banco_nombre: str = Field(..., description="Nombre del banco (join desnormalizado).")
    tipo_cuenta: str = Field(..., description="Tipo de cuenta: corriente, ahorros, etc.")
    nombre_titular: str = Field(..., description="Nombre del titular del instrumento de pago.")
    valor_total: Decimal = Field(..., description="Monto total del pago.")
    distribuciones: list[ReportePagoDistribucionRow] = Field(..., description="Distribución entre entregas.")


class ReportePagosResponse(BaseModel):
    total_pagos: int = Field(..., description="Total de pagos en la respuesta.")
    valor_total: Decimal = Field(..., description="Suma de valor_total de todos los pagos.")
